   ...
   return (
      <>
         <AnimatePresence>
            {active && (
               <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="fixed inset-0 z-10 h-full w-full bg-white/10 backdrop-blur-sm dark:bg-black/10"
               />
            )}
         </AnimatePresence>
         <AnimatePresence>
         ...
         <motion.div
            ...
            className={cn(
               "flex cursor-pointer flex-col items-center justify-between rounded-2xl border-1 border-primary/30 shadow-xl shadow-primary/10 bg-card p-3",
               className
            )}
         >
